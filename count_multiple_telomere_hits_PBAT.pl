...
foreach my $infile (@ARGV){
  ...
  my %counts_TTAAAA; # as this is PBAT seq data we need to look for the reverse complement of TTTTAA
  ...
  while (1){
    ...
    print  "Summary for TTAAAA\n";
    print OUT "Summary for TTAAAA\n";
    print OUT "\t$infile\n";
    foreach my $occurrence(sort {$a<=>$b} keys %counts_TTAAAA){
	print "$occurrence\t$counts_TTAAAA{$occurrence}\n";
	print OUT "$occurrence\t$counts_TTAAAA{$occurrence}\n";
    }
