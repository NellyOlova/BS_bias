...
chdir ("/data/public/Genomes/Human/GRCh37/") or die "Can't move to folder /data/public/Genomes/Human/GRCh37/: $!";
...
my $outfile = 'Human_genome_sequence_composition_4_bp.txt';
chdir ("/data/Projects/human_repeat_genome/") or die "Can't move to folder /data/Projects/human_repeat_genome/: $!";
open (OUT,'>',$outfile) or die $!;

print "\nNumber of different words found in the human genome for the word size ## $word_size ##: ",scalar(keys %freqs),"\n";
print OUT "Number of different words found in the human genome for the word size ## $word_size ##: ",scalar(keys %freqs),"\n";
...
sub read_sequence_file{
  ...
  open (HUMAN_CHROMOSOME,$filename)or die "Can't open $filename: $!";
  $_ = <HUMAN_CHROMOSOME>; #fasta header
  while (<HUMAN_CHROMOSOME>){
    chomp;
    $sequence .= uc$_;
  }
  close HUMAN_CHROMOSOME or die $!;
  return $sequence;
}
